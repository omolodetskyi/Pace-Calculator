...
public class Controller {
	...
	public void CalcFlow(PaceCalc pacecalc, CLUI cli) {
		...
		case 1: // if user selected pace calc
			...
			paceStr = parser.parseFromPace(paceInt); // parse pace from int to
														// string
			...
			while (!answer.equals("yes") && !answer.equals("no")) { // in cycle
																	// until
																	// answer is
																	// yes or no
		...
		case 2: // if user selected time calculation
			...
			cli.showCalculatedTime(timeStr); // show result of time calculation
			cli.showContinueMsg(); // show continue message with yes no no
			answer = userInput.next(); // take answer from user input
			while (!answer.equals("yes") && !answer.equals("no")) { // in cycle
																	// until
		...
		case 3: // if user selected distance calculation
			...
			distanceStr = parser.parseFromDistance(distanceInt); // parse
																	// distance
																	// to string
			cli.showCalculatedDistance(distanceStr); // show result of distance
														// calculation
			cli.showContinueMsg(); // show continue message with yes no no
			answer = userInput.next(); // take answer from user input
			while (!answer.equals("yes") && !answer.equals("no")) { // in cycle
			...
			break;
		case 0:
			cli.showExitQuestionMsg(); // show exit message with yes/no question
			answer = userInput.next(); // take answer from user input
			while (!answer.equals("yes") && !answer.equals("no")) { // in cycle
																	// until
																	// answer is
		...
		default: // if answer userinput is different than 1,2,3,0
			cli.showErrorMsg(); // show error message
			cli.showSelectionMsg(); // ask user to select needed action again
			break;
		}
		userInput.close();
	}
